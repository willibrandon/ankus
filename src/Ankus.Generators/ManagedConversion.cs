...
internal static class ManagedConversion
{
    ...
    internal static string Read(FunctionType type, string slot, string numericSuffix, bool borrowVarlena = false)
    {
        ...
        {
            _ when type.DatumType is not null => slot + ".ReadMapped<" + type.Managed + ">()",
    ...
    internal static string Write(FunctionType result, string value, string target, string numericSuffix)
    {
        ...
        {
            _ when result.DatumType is not null => $"*{target} = global::Ankus.NativeValue.FromMapped<{result.Managed}>({value});",
