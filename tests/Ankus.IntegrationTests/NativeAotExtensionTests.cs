...
public sealed class NativeAotExtensionTests(TestContext context)
{
    ...
    /// <summary>
    /// Verifies signed datums and the sample's deliberately unchecked integer arithmetic.
    /// </summary>
    ...
    public Task AddReturnsExpectedInteger(int left, int right, int expected)
        ...
        {
            await using var command = new NpgsqlCommand("SELECT public.ankus_add($1, $2)", connection, transaction);
